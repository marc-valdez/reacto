...
def draw_image(win: Window, base_path: Path, filename: str, color: str, opacity=0.33):
    ...
    image = ImageStim(
        win, image=png, units="pix", size=(512, 512), color=(color), opacity=opacity
    )
    image.draw()
    ...
    for i, description in enumerate(descriptions):
        ...
        shadow_message = TextStim(
            ...
            pos=(
                0.002,
                -0.102,
            ),  # Position is shifted slightly right (0.004) and down (-0.004)
            color="black",
            opacity=0.7,
        )  # Slight transparency for realism
        main_message = TextStim(
            win, text=full_text, pos=(0, -0.1), height=0.05, color="white"
        )

        images[i].draw()
        if i == 2:
            draw_image(win, base_path, "checkmark-circle-svgrepo-com.png", "green", 0.5)
        else:
...
def mini_test(win: Window, clips: list[str], movies: dict, sounds: dict = {}):
    countdown_manager = CountdownManager()

    message = TextStim(
        win, text="Mini-Test: React to the stimulus in three trials.", height=0.05
    )
    message.draw()
    press_space_to_continue(win)
...
def transition_to_test(win: Window, base_path: Path):
    message = TextStim(
        win,
        text=(
    ...
    while True:
        ...
        if keys and "r" in keys:
            run_tutorial(win, base_path)
            break
        elif keys and "p" in keys:
            win.flip()
            break
...
def confirm_tutorial(win: Window):
    ...
    while True:
        ...
        if keys and "y" in keys:
            return True
        elif keys and "n" in keys:
...
def run_tutorial(win: Window, base_path: Path):
    ...
    transition_to_test(win, base_path)
