...
def export_results(results: list, test_duration: float, client: Client | None = None):
    """Export results to JSON file."""
    # Write to JSON
    with open("results.json", "w") as json_file:
        json.dump(
            {"test_duration": test_duration, "results": results}, json_file, indent=4
        )
    ...
    try:
        if client is None:
            ...
            return
        client.rpc("submit_experiment", {"payload": results}).execute()
