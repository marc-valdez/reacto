...
def display_result_screen(win: Window, rt_ms: float, verdict: str):
    ...
    main_stim = TextStim(
        win, text=main_text, pos=(0, 0.3), color=main_color, height=0.2
    )
    subtitle_stim = TextStim(
        win, text=subtitle_text, pos=(0, 0.1), color="white", height=0.05
    )
    next_stim = TextStim(
        win, text="Press SPACE to continue", pos=(0, -0.2), color="white", height=0.05
    )
    main_stim.draw()
    subtitle_stim.draw()
    next_stim.draw()
...
def display_final_screen(win: Window, averages: dict):
    ...
    if not averages:
        ...
        # Total average
        total_avg = (
            sum([sum(times) / len(times) for times in averages.values() if times])
            / len(averages)
            if averages
            else 0
        )
    ...
    main_stim = TextStim(
        win, text=main_text, pos=(0, 0.3), color=main_color, height=0.25
    )
    subtitle_stim = TextStim(
        win, text="Average Reaction Time", pos=(0, 0.1), color="white", height=0.05
    )
    detail_stim = TextStim(
        win, text=detail_text, pos=(0, -0.1), color="white", height=0.05
    )
    exit_stim = TextStim(
        win, text="Press SPACE to exit", pos=(0, -0.3), color="white", height=0.05
    )
